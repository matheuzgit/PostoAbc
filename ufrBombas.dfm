object FrmBombas: TFrmBombas
  Left = 0
  Top = 0
  Caption = 'Bombas de combust'#237'vel'
  ClientHeight = 383
  ClientWidth = 919
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Label10: TLabel
    Left = 127
    Top = 64
    Width = 91
    Height = 16
    Caption = 'Quantidade(ML)'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Label6: TLabel
    Left = 135
    Top = 72
    Width = 91
    Height = 16
    Caption = 'Quantidade(ML)'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWhite
    Font.Height = -13
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 919
    Height = 383
    Align = alClient
    TabOrder = 0
    object Bomba1: TPanel
      Left = 0
      Top = 0
      Width = 233
      Height = 383
      Color = clLime
      ParentBackground = False
      TabOrder = 0
      object Label2: TLabel
        Left = 96
        Top = 0
        Width = 24
        Height = 25
        Caption = 'B1'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -21
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label11: TLabel
        Left = 53
        Top = 56
        Width = 30
        Height = 16
        Caption = 'Valor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label12: TLabel
        Left = 119
        Top = 56
        Width = 91
        Height = 16
        Caption = 'Quantidade(ML)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object BtnBomba1: TButton
        Left = 80
        Top = 148
        Width = 75
        Height = 25
        Caption = 'Abastecer'
        TabOrder = 0
        OnClick = BtnBomba1Click
      end
      object MemoNotab1: TMemo
        Left = 25
        Top = 224
        Width = 185
        Height = 105
        Lines.Strings = (
          'MemoNotab1')
        TabOrder = 1
      end
      object EdtBomba1: TEdit
        Left = 53
        Top = 31
        Width = 121
        Height = 21
        TabOrder = 2
      end
    end
    object Bomba2: TPanel
      Left = 232
      Top = 0
      Width = 233
      Height = 383
      Color = clRed
      ParentBackground = False
      TabOrder = 1
      object Label3: TLabel
        Left = 104
        Top = 0
        Width = 24
        Height = 25
        Caption = 'B2'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -21
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label9: TLabel
        Left = 54
        Top = 59
        Width = 30
        Height = 16
        Caption = 'Valor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label13: TLabel
        Left = 127
        Top = 58
        Width = 91
        Height = 16
        Caption = 'Quantidade(ML)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object MemoNotab2: TMemo
        Left = 32
        Top = 224
        Width = 185
        Height = 105
        Lines.Strings = (
          'MemoNota')
        TabOrder = 0
      end
      object BtnAbastecerB2: TButton
        Left = 88
        Top = 148
        Width = 75
        Height = 25
        Caption = 'Abastecer'
        TabOrder = 1
        OnClick = BtnAbastecerB2Click
      end
      object MaskB2: TMaskEdit
        Left = 48
        Top = 78
        Width = 73
        Height = 21
        TabOrder = 2
        Text = 'R$:'
      end
      object EdtB2: TEdit
        Left = 131
        Top = 78
        Width = 65
        Height = 21
        TabOrder = 3
      end
      object EdtBomba2: TEdit
        Left = 62
        Top = 31
        Width = 121
        Height = 21
        TabOrder = 4
      end
    end
    object Bomba3: TPanel
      Left = 463
      Top = 0
      Width = 233
      Height = 383
      Color = clYellow
      ParentBackground = False
      TabOrder = 2
      object Label4: TLabel
        Left = 104
        Top = 0
        Width = 24
        Height = 25
        Caption = 'B3'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -21
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label7: TLabel
        Left = 118
        Top = 56
        Width = 91
        Height = 16
        Caption = 'Quantidade(ML)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label8: TLabel
        Left = 53
        Top = 56
        Width = 30
        Height = 16
        Caption = 'Valor'
        Color = clBlack
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object MemoNotab3: TMemo
        Left = 24
        Top = 224
        Width = 185
        Height = 105
        Lines.Strings = (
          'MemoNota')
        TabOrder = 0
      end
      object BtnAbastecerB3: TButton
        Left = 84
        Top = 148
        Width = 75
        Height = 25
        Caption = 'Abastecer'
        TabOrder = 1
        OnClick = BtnAbastecerB3Click
      end
      object MaskB3: TMaskEdit
        Left = 47
        Top = 78
        Width = 73
        Height = 21
        TabOrder = 2
        Text = 'R$:'
      end
      object EdtB3: TEdit
        Left = 127
        Top = 78
        Width = 65
        Height = 21
        TabOrder = 3
      end
      object EdtBomba3: TEdit
        Left = 62
        Top = 29
        Width = 121
        Height = 21
        TabOrder = 4
      end
    end
    object Boma4: TPanel
      Left = 694
      Top = 0
      Width = 233
      Height = 383
      Color = clBlue
      ParentBackground = False
      TabOrder = 3
      object Label1: TLabel
        Left = 96
        Top = 0
        Width = 24
        Height = 25
        Caption = 'B4'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -21
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label5: TLabel
        Left = 53
        Top = 56
        Width = 30
        Height = 16
        Caption = 'Valor'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object Label14: TLabel
        Left = 121
        Top = 56
        Width = 91
        Height = 16
        Caption = 'Quantidade(ML)'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWhite
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = []
        ParentFont = False
      end
      object MemoNotab4: TMemo
        Left = 17
        Top = 224
        Width = 185
        Height = 105
        Lines.Strings = (
          'MemoNota')
        TabOrder = 0
      end
      object BtnAbastecerB4: TButton
        Left = 81
        Top = 148
        Width = 75
        Height = 25
        Caption = 'Abastecer'
        TabOrder = 1
        OnClick = BtnAbastecerB4Click
      end
      object MaskB4: TMaskEdit
        Left = 42
        Top = 78
        Width = 73
        Height = 21
        TabOrder = 2
        Text = 'R$:'
      end
      object EdtB4: TEdit
        Left = 128
        Top = 78
        Width = 65
        Height = 21
        TabOrder = 3
      end
      object EdtBomba4: TEdit
        Left = 54
        Top = 29
        Width = 121
        Height = 21
        TabOrder = 4
      end
    end
  end
  object MaskB1: TMaskEdit
    Left = 40
    Top = 78
    Width = 73
    Height = 21
    TabOrder = 1
    Text = 'R$:'
  end
  object EdtB1: TEdit
    Left = 126
    Top = 78
    Width = 65
    Height = 21
    TabOrder = 2
  end
  object QryBomba: TFDQuery
    Connection = DataModule1.FDconSYSCombo
    SQL.Strings = (
      'select * from TBBOMBAS WHERE BOMBA =:BOMBA')
    Left = 296
    Top = 336
    ParamData = <
      item
        Name = 'BOMBA'
        DataType = ftString
        ParamType = ptInput
        Size = 10
      end>
    object QryBombaBOMBA: TStringField
      FieldName = 'BOMBA'
      Origin = 'BOMBA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 10
    end
    object QryBombaID_TANQUE: TStringField
      FieldName = 'ID_TANQUE'
      Origin = 'ID_TANQUE'
      Required = True
    end
    object QryBombaIMPOSTO: TSingleField
      FieldName = 'IMPOSTO'
      Origin = 'IMPOSTO'
      Required = True
    end
    object QryBombaVALOR_LITRO: TSingleField
      FieldName = 'VALOR_LITRO'
      Origin = 'VALOR_LITRO'
      Required = True
    end
  end
  object QryTanque: TFDQuery
    Connection = DataModule1.FDconSYSCombo
    SQL.Strings = (
      'SELECT * FROM TBTANQUE')
    Left = 216
    Top = 328
    object QryTanqueID_TANQUE: TStringField
      FieldName = 'ID_TANQUE'
      Origin = 'ID_TANQUE'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object QryTanqueTIPO_COMBUSTIVEL: TStringField
      FieldName = 'TIPO_COMBUSTIVEL'
      Origin = 'TIPO_COMBUSTIVEL'
      Required = True
    end
    object QryTanqueLITROS: TSingleField
      FieldName = 'LITROS'
      Origin = 'LITROS'
      Required = True
    end
  end
  object QryAbastecimento: TFDQuery
    Connection = DataModule1.FDconSYSCombo
    SQL.Strings = (
      'SELECT * FROM TBABASTECIMENTO')
    Left = 448
    Top = 336
    object QryAbastecimentoID_TANQUE: TStringField
      FieldName = 'ID_TANQUE'
      Origin = 'ID_TANQUE'
      Required = True
    end
    object QryAbastecimentoDATAHORA: TSQLTimeStampField
      FieldName = 'DATAHORA'
      Origin = 'DATAHORA'
      Required = True
    end
    object QryAbastecimentoVALOR_PAGO: TSingleField
      FieldName = 'VALOR_PAGO'
      Origin = 'VALOR_PAGO'
      Required = True
    end
    object QryAbastecimentoQUANTIDADE: TSingleField
      FieldName = 'QUANTIDADE'
      Origin = 'QUANTIDADE'
      Required = True
    end
    object QryAbastecimentoIMPOSTO_PAGO: TSingleField
      FieldName = 'IMPOSTO_PAGO'
      Origin = 'IMPOSTO_PAGO'
      Required = True
    end
    object QryAbastecimentoBOMBA: TStringField
      FieldName = 'BOMBA'
      Origin = 'BOMBA'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Size = 10
    end
  end
  object QryVolumeTanque: TFDQuery
    Connection = DataModule1.FDconSYSCombo
    Left = 512
    Top = 256
  end
end
